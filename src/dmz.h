 ...
 */
#define DMZ_MAGIC                                                              \
  ((((unsigned int)('D')) << 24) | (((unsigned int)('Z')) << 16) |             \
   (((unsigned int)('B')) << 8) | ((unsigned int)('D')))
 ...
 * Bitmap operations.
 */
static inline int dmz_test_bit(__u8 *bitmap, unsigned int bit) {
  return bitmap[bit >> 3] & (1 << (bit & 0x7));
}
static inline void dmz_set_bit(__u8 *bitmap, unsigned int bit) {
  bitmap[bit >> 3] |= 1 << (bit & 0x7);
}
static inline void dmz_clear_bit(__u8 *bitmap, unsigned int bit) {
  bitmap[bit >> 3] &= ~(1 << (bit & 0x7));
}
...
#define DMZ_MSET_VALID                                                         \
  (DMZ_MSET_SB_VALID | DMZ_MSET_MAP_VALID | DMZ_MSET_BITMAP_VALID)
...
static inline bool dmz_zone_is_cache(struct dmz_dev *dev,
...
#define dmz_zone_id(dev, zone)                                                 \
  ((unsigned int)(dmz_zone_sector(zone) / (dev)->zone_nr_sectors))
...
extern struct dmz_block_dev *dmz_block_to_bdev(struct dmz_dev *dev, __u64 block,
                                               __u64 *ret_block);
extern struct dmz_block_dev *
