...
#define dmz_msg(dev, ind, format, args...) printf("%*s" format, ind, "", ##args)
#define dmz_err(dev, ind, format, args...) printf("%*s" format, ind, "", ##args)
...
static int dmz_read_zone_bitmap(struct dmz_dev *dev, struct dmz_meta_set *mset,
  ...
  bitmap_block = mset->bitmap_block + (zone_id * dev->zone_nr_bitmap_blocks);
  for (b = 0; b < dev->zone_nr_bitmap_blocks; b++) {
    ret = dmz_read_block(dev, bitmap_block + b,
                         bitmap_buf + (b * DMZ_BLOCK_SIZE));
    if (ret != 0) {
      fprintf(stderr, "Read zone %u bitmap block %llu failed\n", zone_id,
              bitmap_block + b);
      free(bitmap_buf);
      return -1;
    }
...
static int dmz_write_zone_bitmap(struct dmz_dev *dev, struct dmz_meta_set *mset,
  ...
  bitmap_block = mset->bitmap_block + (zone_id * dev->zone_nr_bitmap_blocks);
  for (b = 0; b < dev->zone_nr_bitmap_blocks; b++) {
    ret = dmz_write_block(dev, bitmap_block + b, buf + (b * DMZ_BLOCK_SIZE));
    if (ret != 0) {
      fprintf(stderr, "Write zone %u bitmap block %llu failed\n", zone_id,
              bitmap_block + b);
      return -1;
    }
  }
...
static int dmz_read_map_blocks(struct dmz_dev *dev, struct dmz_meta_set *mset) {
  ...
  for (b = 0; b < dev->nr_map_blocks; b++) {
    ...
    if (ret != 0) {
      fprintf(stderr, "Read map block %llu failed\n", mset->map_block + b);
      free(mset->map_buf);
      mset->map_buf = NULL;
      return -1;
    }
 ...
 */
static int dmz_write_map_blocks(struct dmz_dev *dev,
  ...
  for (b = 0; b < dev->nr_map_blocks; b++) {
    ret = dmz_write_block(dev, mset->map_block + b,
                          mset->map_buf + (b * DMZ_BLOCK_SIZE));
    if (ret != 0) {
      fprintf(stderr, "Write map block %llu failed\n", mset->map_block + b);
      return -1;
    }
  }
...
static void dmz_get_chunk_mapping(struct dmz_dev *dev,
                                  struct dmz_meta_set *mset, unsigned int chunk,
                                  unsigned int *dzone_id,
                                  unsigned int *bzone_id) {
  struct dm_zoned_map *map;
  unsigned int map_idx = chunk & DMZ_MAP_ENTRIES_MASK;

  map = (struct dm_zoned_map *)(mset->map_buf +
                                ((chunk / DMZ_MAP_ENTRIES) * DMZ_BLOCK_SIZE));
...
static int dmz_validate_chunk_mapping(struct dmz_dev *dev,
                                      struct dmz_meta_set *mset,
                                      unsigned int chunk, unsigned int dzone_id,
                                      unsigned int bzone_id) {
  ...
  for (c = 0; c < dev->nr_chunks; c++) {
    ...
    /* Check data zone */
    if (dzid == dzone_id || bzid == dzone_id) {
      dmz_err(dev, ind, "Chunk %u: data zone %u used by chunk %u\n", chunk,
              dzone_id, c);
      errors++;
      if (dmz_repair_dev(dev)) {
        dmz_set_chunk_mapping(dev, mset, c, DMZ_MAP_UNMAPPED, DMZ_MAP_UNMAPPED);
        continue;
      }
    }
    ...
    /* Check buffer zone */
    if (dzid == bzone_id || bzid == bzone_id) {
      dmz_err(dev, ind, "Chunk %u: buffer zone %u used by chunk %u\n", chunk,
              bzone_id, c);
      errors++;
      if (dmz_repair_dev(dev)) {
        dmz_set_chunk_mapping(dev, mset, c, DMZ_MAP_UNMAPPED, DMZ_MAP_UNMAPPED);
...
static int dmz_check_chunk_mapping(struct dmz_dev *dev,
  ...
  if (dzone_id != DMZ_MAP_UNMAPPED) {
    ...
    if (dzone_id >= dev->nr_zones) {
      dmz_err(dev, ind, "Chunk %u: invalid data zone %u\n", chunk, dzone_id);
      errors++;
      if (dmz_repair_dev(dev))
  ...
  if (dzone_id == DMZ_MAP_UNMAPPED) {
    if (bzone_id != DMZ_MAP_UNMAPPED) {
      ...
      dmz_err(dev, ind, "Chunk %u: unmapped but buffer zone %u set\n", chunk,
              bzone_id);
      errors++;
      if (dmz_repair_dev(dev))
  ...
  if (bzone_id == DMZ_MAP_UNMAPPED) {
    ...
    if (dmz_zone_seq_req(dzone) && dmz_zone_empty(dzone)) {
      dmz_err(dev, ind, "Chunk %u: mapped to empty seq req zone %u\n", chunk,
              dzone_id);
      if (dmz_repair_dev(dev)) {
  ...
  if (bzone_id == dzone_id || bzone_id >= dev->nr_zones) {
    dmz_err(dev, ind, "Chunk %u: invalid buffer zone %u\n", chunk, dzone_id);
    errors++;
    if (dmz_repair_dev(dev))
      bzone_id = DMZ_MAP_UNMAPPED;
    goto out;
  ...
  if (!dmz_zone_is_cache(dev, bzone)) {
    dmz_err(dev, ind, "Chunk %u: buffer zone %u is not a cache zone\n", chunk,
            bzone_id);
    errors++;
    if (dmz_repair_dev(dev))
      bzone_id = DMZ_MAP_UNMAPPED;
...
out:
  if (dmz_repair_dev(dev) && errors)
    dmz_set_chunk_mapping(dev, mset, chunk, dzone_id, bzone_id);

  errors += dmz_validate_chunk_mapping(dev, mset, chunk, dzone_id, bzone_id);
 ...
 */
static int dmz_check_mapping(struct dmz_dev *dev, struct dmz_meta_set *mset) {
  ...
  if (mset->error_count == 0) {
    dmz_msg(dev, ind + 2, "No error: %u mapped chunk%s (%u buffered) checked\n",
...
static int dmz_check_unmapped_zone_bitmap(struct dmz_dev *dev,
  ...
  for (b = 0; b < dev->zone_nr_blocks; b++) {
    if (!dmz_test_bit(buf, b))
      continue;
    bad_bits++;
    dmz_verr(dev, ind, "Zone %u: unmapped zone but block %u valid\n", zone_id,
             b);
    ind = 4;
    errors++;
    if (dmz_repair_dev(dev))
  ...
  if (bad_bits)
    dmz_verr(dev, ind, "Zone %u: unmapped zone but %u block%s valid\n", zone_id,
  ...
  if (dmz_zone_seq_req(zone) && zone->wp != zone->start) {
    dmz_err(dev, ind,
            ...
            "(wp at +%u blocks)\n",
            zone_id, (unsigned int)dmz_sect2blk(zone->wp - zone->start));
    errors++;
    if (dmz_repair_dev(dev))
      /* Reset zone */
...
static int dmz_check_mapped_zone_bitmap(struct dmz_dev *dev,
  ...
  for (b = wp_block; b < dev->zone_nr_blocks; b++) {
    if (!dmz_test_bit(dbuf, b))
      continue;
    dmz_verr(dev, ind, "Zone %u: block %u valid after zone wp block %u\n",
             dzone_id, b, wp_block);
    dzone_weight++;
    bad_bits++;
    errors++;
  ...
  if (bad_bits)
    dmz_err(dev, ind,
            ...
            dzone_id, chunk, dzone_weight, bad_bits, dmz_plural(bad_bits),
            wp_block);
...
static unsigned int dmz_get_zone_mapping(struct dmz_dev *dev,
  ...
  for (c = 0; c < dev->nr_chunks; c++) {
    dmz_get_chunk_mapping(dev, mset, c, &dzid, &bzid);
    if (dmz_zone_id(dev, zone) == dzid || dmz_zone_id(dev, zone) == bzid) {
      *bzone_id = bzid;
      return c;
    }
  }
...
static int dmz_check_bitmaps(struct dmz_dev *dev, struct dmz_meta_set *mset) {
  struct blk_zone *zone;
  ...
  for (i = 0; i < dev->nr_zones; i++) {
    ...
    if (chunk == DMZ_MAP_UNMAPPED) {
      ...
      ret = dmz_check_mapped_zone_bitmap(dev, mset, chunk, zone, bzone_id);
      if (ret != 0)
        return -1;
      mapped_zones++;
    }
  ...
  if (mset->error_count == 0) {
    dmz_msg(dev, ind + 2,
            ...
            "checked\n",
            unmapped_zones, dmz_plural(unmapped_zones), mapped_zones,
            dmz_plural(mapped_zones));
  ...
  dmz_msg(dev, ind + 2,
          ...
          "checked\n",
          mset->error_count, (mset->error_count > 1) ? "s" : "", unmapped_zones,
          dmz_plural(unmapped_zones), mapped_zones, dmz_plural(mapped_zones));
 ...
 */
static int dmz_check_meta(struct dmz_dev *dev, struct dmz_meta_set *mset) {
  int ret;

  /* Check zone mapping */
  ret = dmz_check_mapping(dev, mset);
  if (ret != 0) {
    fprintf(stderr, "Check %s metadata set mapping failed\n",
  ...
  if (ret != 0) {
    fprintf(stderr, "Check %s metadata set zone bitmaps failed\n",
            (mset->id == 0) ? "primary" : "secondary");
    return -1;
  }

  if (mset->flags != DMZ_MSET_VALID)
    dmz_msg(dev, 2, "%s metadata set: %u error%s found%s\n",
            (mset->id == 0) ? "Primary" : "Secondary", mset->total_error_count,
...
static int dmz_check_sb(struct dmz_dev *dev, struct dmz_meta_set *mset) {
  ...
  if (__le32_to_cpu(sb->magic) != DMZ_MAGIC) {
    dmz_err(dev, 0, "invalid magic (expected 0x%08x read 0x%08x)\n", DMZ_MAGIC,
  ...
  if (calculated_crc != stored_crc) {
    dmz_err(dev, 0, "invalid crc (expected 0x%08x, read 0x%08x)\n",
  ...
  if (__le32_to_cpu(sb->version) > DMZ_META_VER) {
    dmz_err(dev, 0, "invalid version (expected 0x%x, read 0x%x)\n",
  ...
  if (__le32_to_cpu(sb->version) > 1) {
    ...
    if (uuid_is_null(sb->dmz_uuid)) {
      ...
      dmz_err(dev, 0, "DM-Zoned UUID mismatch (expected %s, read %s)\n",
    ...
    if (!strlen((const char *)sb->dmz_label)) {
      ...
      dmz_err(dev, 0, "DM-Zoned label mismatch (expected %s, read %s)\n",
    ...
    if (mset->id < 2) {
      ...
      if (uuid_is_null(bdev->uuid)) {
        ...
        uuid_unparse(sb->dmz_uuid, sb_uuid_buf);
        dmz_err(dev, 0, "Device UUID mismatch (expected %s, read %s)\n",
                dev_uuid_buf, sb_uuid_buf);
        goto err;
      }
    } else {
  ...
  if (__le64_to_cpu(sb->sb_block) != mset->sb_block) {
    dmz_err(dev, 0, "invalid location (expected %llu, read %llu)\n",
  ...
  dev->nr_meta_zones =
      DIV_ROUND_UP(__le64_to_cpu(sb->nr_meta_blocks), dev->zone_nr_blocks);
  dev->nr_chunks =
      dev->nr_usable_zones - ((dev->nr_meta_zones * 2) + dev->nr_reserved_seq);
 ...
 * Print format info.
 */
static void dmz_check_print_format(struct dmz_dev *dev, int ind) {
  ...
  dmz_msg(dev, ind + 2, "Using %u zones per meta-data set (%u total)\n",
          dev->nr_meta_zones, dev->total_nr_meta_zones);

  dev->nr_cache_zones -= dev->total_nr_meta_zones;
  nr_data_zones =
      dev->nr_usable_zones -
      (dev->total_nr_meta_zones + dev->nr_cache_zones + dev->nr_reserved_seq);
  ...
  dmz_msg(dev, ind, "%u sequential zone%s reserved for reclaim\n",
          dev->nr_reserved_seq, dev->nr_reserved_seq > 1 ? "s" : "");
}
...
static int dmz_check_superblocks(struct dmz_dev *dev,
  ...
  if (dmz_locate_metadata(dev) < 0) {
    fprintf(stderr, "Failed to locate metadata\n");
    return -1;
  }
  ...
  if (mset[0].flags & DMZ_MSET_SB_VALID) {
    ...
    mset[1].sb_block =
        mset[0].sb_block + (dev->nr_meta_zones * dev->zone_nr_blocks);
    ...
    for (i = 0; i < dev->max_nr_meta_zones - 1; i++) {
      ret = dmz_read_block(dev, mset[1].sb_block, mset[1].buf);
      if (ret != 0)
        continue;
  ...
  dmz_msg(dev, ind, "Secondary metadata set at block %llu (zone %u)\n",
  ...
  if (mset[1].flags & DMZ_MSET_SB_VALID && !(mset[0].flags & DMZ_MSET_SB_VALID))
    dmz_check_print_format(dev, ind + 2);
...
static int dmz_check_tertiary_superblocks(struct dmz_dev *dev) {
  ...
  for (i = 1; i < dev->nr_bdev; i++) {
    ...
    dmz_msg(dev, 2, "Tertiary superblock at block %llu (zone %u)\n",
...
static int dmz_compare_meta(struct dmz_dev *dev,
  ...
  dmz_msg(dev, ind, "Validating %s metadata set against %s metadata set...\n",
          (mset->id == 0) ? "primary" : "secondary",
  ...
  for (b = 1; b < dev->nr_meta_blocks; b++) {
    ...
    ret = dmz_read_block(dev, check_mset->sb_block + b, check_mset->buf);
    if (ret != 0)
      return -1;
    ...
    if (memcmp(check_mset->buf, mset->buf, DMZ_BLOCK_SIZE) != 0) {
      dmz_verr(dev, ind + 2, "Block %llu differ\n", mset->sb_block + b);
      mset->error_count++;
    }
  }

  if (mset->error_count == 0) {
    dmz_msg(dev, ind + 2, "No error: %u blocks checked\n", dev->nr_meta_blocks);
    mset->flags = DMZ_MSET_VALID;
  } else {
    dmz_err(dev, ind + 2, "%u block%s differ\n", mset->error_count,
            dmz_plural(mset->error_count));
    mset->total_error_count += mset->error_count;
  }
...
int dmz_check(struct dmz_dev *dev) {
  ...
  if (ret != 0) {
    fprintf(stderr, "Check device superblocks failed\n");
    return -1;
  }
  ...
  if (ret != 0) {
    fprintf(stderr, "Check %s metadata set failed\n",
            (check_mset->id == 0) ? "primary" : "secondary");
    ret = -1;
    goto out;
  }
  ...
  if (mset[0].flags == DMZ_MSET_VALID && mset[1].flags == DMZ_MSET_VALID &&
      mset[2].flags == DMZ_MSET_VALID)
    dmz_msg(dev, 0, "Done\n");
  else
...
static int dmz_repair_sync_meta(struct dmz_dev *dev,
  ...
  if (dst_mset->flags == DMZ_MSET_VALID && src_mset->gen == dst_mset->gen)
    /* Nothing to do */
    return 0;

  dmz_msg(dev, 0, "Syncing %s metadata set to %s metadata set...\n",
...
int dmz_repair(struct dmz_dev *dev) {
  ...
  dmz_msg(dev, 0, "Using %s metadata set for checks\n",
          (check_mset->id == 0) ? "primary" : "secondary");
  ...
  if (check_mset->total_error_count)
    ...
    dmz_msg(dev, 0, "%u error%s found and repaired\n",
  ...
  if (ret != 0) {
    fprintf(stderr, "Sync %s metadata set to %s metadata set failed\n",
            (check_mset->id == 0) ? "primary" : "secondary",
            (id == 0) ? "primary" : "secondary");
    return -1;
  }
