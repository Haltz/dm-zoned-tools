 ...
 */
struct dmz_block_dev *dmz_block_to_bdev(struct dmz_dev *dev, __u64 block,
                                        __u64 *ret_block) {
  int i;
 ...
 */
struct dmz_block_dev *dmz_sector_to_bdev(struct dmz_dev *dev, __u64 sector,
                                         __u64 *ret_sector) {
  int i;
 ...
 */
static void dmz_print_zone(struct dmz_dev *dev, struct dmz_block_dev *bdev,
                           struct blk_zone *zone) {

  if (dmz_zone_cond(zone) == BLK_ZONE_COND_READONLY) {
    printf("Zone %06u (%s): readonly %s zone\n", dmz_zone_id(dev, zone),
           bdev->name, dmz_zone_cond_str(zone));
    return;
  }
  ...
  if (dmz_zone_conv(zone)) {
    printf("Zone %06u (%s): Conventional, cond 0x%x (%s), "
           ...
           dmz_zone_id(dev, zone), bdev->name, dmz_zone_cond(zone),
           dmz_zone_cond_str(zone), dmz_zone_sector(zone),
  ...
  printf("Zone %06u (%s): type 0x%x (%s), cond 0x%x (%s), need_reset %d, "
         ...
         dmz_zone_id(dev, zone), bdev->name, dmz_zone_type(zone),
         dmz_zone_type_str(zone), dmz_zone_cond(zone), dmz_zone_cond_str(zone),
         dmz_zone_need_reset(zone), dmz_zone_non_seq(zone),
         dmz_zone_sector(zone), dmz_zone_length(zone),
...
int dmz_get_dev_zones(struct dmz_dev *dev) {
  ...
  rep_max_zones = (DMZ_REPORT_ZONES_BUFSZ - sizeof(struct blk_zone_report)) /
...
int dmz_read_block(struct dmz_dev *dev, __u64 block, __u8 *buf) {
  ...
  if (ret != DMZ_BLOCK_SIZE) {
    fprintf(stderr, "%s: Read block %llu failed %d (%s)\n", bdev->name,
            read_block, errno, strerror(errno));
    return -1;
  }
