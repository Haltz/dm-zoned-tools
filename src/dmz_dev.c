 ...
 */
struct dmz_block_dev *dmz_block_to_bdev(struct dmz_dev *dev, __u64 block, __u64 *ret_block) {
	int i;
 ...
 */
struct dmz_block_dev *dmz_sector_to_bdev(struct dmz_dev *dev, __u64 sector, __u64 *ret_sector) {
	int i;

	*ret_sector = sector;

	for (i = dev->nr_bdev - 1; i >= 0; i--) {
		__u64 sector_offset = dmz_blk2sect(dev->bdev[i].block_offset);
...
static int dmz_dev_busy(struct dmz_block_dev *dev, char *holder) {
	...
	snprintf(path, sizeof(path), "/sys/class/block/%s/holders", dev->name);
...
static int dmz_dev_is_partition(struct dmz_block_dev *dev) {
	...
	len = snprintf(str, sizeof(str), "/sys/class/block/%s/partition", dev->name);
	if (len >= PATH_MAX) {
...
static int dmz_get_dev_model(struct dmz_block_dev *dev) {
	...
	len = snprintf(str, sizeof(str), "/sys/block/%s/queue/zoned", dev->name);
...
static int dmz_get_dev_capacity(struct dmz_block_dev *dev) {
	...
	if (ioctl(dev->fd, BLKGETSIZE64, &dev->capacity) < 0) {
		fprintf(stderr, "%s: Get capacity failed %d (%s)\n", dev->path, errno, strerror(errno));
		return -1;
	}
	dev->capacity >>= 9;
	...
	/* Get zone size */
	snprintf(str, sizeof(str), "/sys/block/%s/queue/chunk_sectors", dev->name);
	file = fopen(str, "r");
	if (!file) {
		fprintf(stderr, "Open %s failed\n", str);
		return -1;
	...
	if (!dev->zone_nr_sectors || (dev->zone_nr_sectors & DMZ_BLOCK_SECTORS_MASK)) {
		fprintf(stderr, "%s: Invalid zone size\n", dev->path);
		return -1;
	}
	dev->zone_nr_blocks = dmz_sect2blk(dev->zone_nr_sectors);

	/* Get number of zones */
	snprintf(str, sizeof(str), "/sys/block/%s/queue/nr_zones", dev->name);
	file = fopen(str, "r");
	if (!file) {
		fprintf(stderr, "Open %s failed\n", str);
		return -1;
 ...
 */
static void dmz_print_zone(struct dmz_dev *dev, struct dmz_block_dev *bdev, struct blk_zone *zone) {
	if (dmz_zone_cond(zone) == BLK_ZONE_COND_READONLY) {
		printf("Zone %06u (%s): readonly %s zone\n", dmz_zone_id(dev, zone), bdev->name, dmz_zone_cond_str(zone));
		return;
	}
	...
	if (dmz_zone_conv(zone)) {
		printf("Zone %06u (%s): Conventional, cond 0x%x (%s), "
		       ...
		       dmz_zone_id(dev, zone), bdev->name, dmz_zone_cond(zone), dmz_zone_cond_str(zone), dmz_zone_sector(zone), dmz_zone_length(zone));
		return;
	}

	printf("Zone %06u (%s): type 0x%x (%s), cond 0x%x (%s), need_reset %d, "
	       "non_seq %d, sector %llu, %llu sectors, wp sector %llu\n",
	       dmz_zone_id(dev, zone), bdev->name, dmz_zone_type(zone), dmz_zone_type_str(zone), dmz_zone_cond(zone), dmz_zone_cond_str(zone), dmz_zone_need_reset(zone), dmz_zone_non_seq(zone), dmz_zone_sector(zone), dmz_zone_length(zone),
...
int dmz_get_dev_zones(struct dmz_dev *dev) {
	...
	rep_max_zones = (DMZ_REPORT_ZONES_BUFSZ - sizeof(struct blk_zone_report)) / sizeof(struct blk_zone);
	...
	while (sector < dev->capacity) {
		...
		for (i = 0; i < rep->nr_zones; i++) {
			/* Check zone size */
			if (dmz_zone_length(blkz) != dev->zone_nr_sectors && dmz_zone_sector(blkz) + dmz_zone_length(blkz) != bdev->capacity) {
				fprintf(stderr, "%s: Invalid zone %u size\n", bdev->name, dmz_zone_id(dev, blkz));
				ret = -1;
				goto out;
			}

			if (nr_zones >= dev->nr_zones) {
				fprintf(stderr, "%s: Invalid zone %u start %llu\n", bdev->name, nr_zones, blkz->start);
				ret = -1;
				goto out;
			}
	...
	if (dev->nr_zones != nr_zones) {
		fprintf(stderr, "%s: Invalid number of zones (expected %u, got %u)\n", dev->label, dev->nr_zones, nr_zones);
		ret = -1;
		goto out;
	}
...
static int dmz_check_overwrite(struct dmz_block_dev *dev) {
	...
	if (ret == 0) {
		fprintf(stderr, "%s appears to contain an existing filesystem (%s)\n", dev->path, type);
		goto out;
	}

	ret = blkid_probe_lookup_value(pr, "PTTYPE", &type, NULL);
	if (ret == 0) {
		fprintf(stderr, "%s appears to contain a partition table (%s)\n", dev->path, type);
		goto out;
	}

	fprintf(stderr, "%s appears to contain something according to blkid\n", dev->path);
	ret = 0;
...
out:
	...
	if (ret == 0)
		...
		fprintf(stderr, "%s: probe failed, cannot detect existing filesystem\n", dev->name);
...
int dmz_read_block(struct dmz_dev *dev, __u64 block, __u8 *buf) {
	__u64 read_block;
	struct dmz_block_dev *bdev = dmz_block_to_bdev(dev, block, &read_block);
	ssize_t ret;

	ret = pread(bdev->fd, (char *)buf, DMZ_BLOCK_SIZE, read_block << DMZ_BLOCK_SHIFT);

	if (ret != DMZ_BLOCK_SIZE) {
		fprintf(stderr, "%s: Read block %llu failed %d (%s)\n", bdev->name, read_block, errno, strerror(errno));
		return -1;
	}
...
int dmz_write_block(struct dmz_dev *dev, __u64 block, __u8 *buf) {
	__u64 write_block;
	struct dmz_block_dev *bdev = dmz_block_to_bdev(dev, block, &write_block);
	ssize_t ret;

	ret = pwrite(bdev->fd, (char *)buf, DMZ_BLOCK_SIZE, write_block << DMZ_BLOCK_SHIFT);
	if (ret != DMZ_BLOCK_SIZE) {
		fprintf(stderr, "%s: Write block %llu failed %d (%s)\n", bdev->name, block, errno, strerror(errno));
		return -1;
	}
