...
int dmz_load_module(const char *modname, int log_level) {
  ...
  kmod_list_foreach(itr, modlist) {
    ...
    ret = kmod_module_probe_insert_module(mod, KMOD_PROBE_APPLY_BLACKLIST, NULL,
                                          NULL, NULL, NULL);
    if (ret) {
      if (ret == KMOD_PROBE_APPLY_BLACKLIST) {
        fprintf(stderr, "Module '%s' is blacklisted\n", modname);
        ret = -ENXIO;
      } else {
        fprintf(stderr, "Module '%s' not loaded, error %d\n", modname, ret);
      }
    } else if (log_level)
  ...
  do {
    ...
    if (!strncmp("zoned", tgt->name, 5)) {
      ...
      case 1:
        ...
        break;
      default:
        fprintf(stderr, "Unsupported dm-zoned version %d.%d.%d\n",
...
int dmz_create_dm(struct dmz_dev *dev) {
  ...
  if (dev->sb_version > 1 && dev->nr_bdev > 1) {
    ...
    for (i = 0; i < dev->nr_bdev; i++) {
      len += snprintf(params + len, 4096 - len, "%s ", dev->bdev[i].path);
    }
  } else {
...
int dmz_check_dm_target(struct dmz_dev *dev, char *dm_dev) {
  ...
  if (strlen(target_type) == 5 && !strncmp(target_type, "zoned", 5)) {
...
static int dmz_load_sb(struct dmz_dev *dev) {
  ...
  if (__le32_to_cpu(sb->magic) != DMZ_MAGIC) {
    fprintf(stderr, "%s: invalid magic (expected 0x%08x read 0x%08x)\n",
  ...
  if (calculated_crc != stored_crc) {
    fprintf(stderr, "%s: invalid crc (expected 0x%08x, read 0x%08x)\n",
...
int dmz_start(struct dmz_dev *dev) {
  ...
  if (dmz_load_sb(dev) < 0) {
    fprintf(stderr, "Failed to load metadata\n");
    return -1;
  }
...
int dmz_stop(struct dmz_dev *dev, char *dm_name) {
  ...
  if (ret < 0) {
    fprintf(stderr, "%s: dm device %s is not a zoned target device\n",
  ...
  if (ret < 0) {
    fprintf(stderr, "%s: could not deactivate %s\n", dev->bdev[0].name,
            dev->label);
    return ret;
  }
