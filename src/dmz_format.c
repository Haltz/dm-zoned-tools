...
int dmz_write_super(struct dmz_dev *dev, __u64 gen, __u64 offset) {
	...
	if (ret < 0)
		fprintf(stderr, "%s: Write super block at block %llu failed\n", bdev->name, bdev_sb_block);
...
static int dmz_write_mapping(struct dmz_dev *dev, __u64 offset) {
	...
	for (i = 0; i < dev->nr_map_blocks; i++) {
		...
		if (ret < 0) {
			fprintf(stderr, "%s: Write mapping block %llu failed\n", dev->label, map_block + i);
			break;
		}
	}
...
static int dmz_write_bitmap(struct dmz_dev *dev, __u64 offset) {
	...
	for (i = 0; i < dev->nr_bitmap_blocks; i++) {
		...
		if (ret < 0) {
			fprintf(stderr, "%s: Write bitmap block %llu failed\n", dev->label, bitmap_block + i);
			break;
		}
	}
...
int dmz_format(struct dmz_dev *dev) {
	...
	if (dev->flags & DMZ_VERBOSE) {
		...
		printf("  Primary meta-data set: %u metadata blocks from block %llu (zone "
		       "%u)\n",
		       dev->nr_meta_blocks, dev->sb_block, dmz_zone_id(dev, dev->sb_zone));
		printf("    Super block at block %llu and %llu\n", dev->sb_block, dev->sb_block + (dev->nr_meta_zones * dev->zone_nr_blocks));
		...
		nr_data_zones = dev->nr_usable_zones - (dev->total_nr_meta_zones + dev->nr_cache_zones + dev->nr_reserved_seq);
		printf("  %u data chunks capacity\n", dev->nr_chunks);
		printf("    %u cache zone%s\n", dev->nr_cache_zones, dev->nr_cache_zones > 1 ? "s" : "");
		printf("    %u data zone%s\n", nr_data_zones, nr_data_zones > 1 ? "s" : "");
	...
	printf("Writing secondary metadata set\tThis is disabled by haltz\n");
	...
	if (dev->sb_version > 1 && dev->nr_bdev > 1) {
		...
		for (i = 1; i < dev->nr_bdev; i++) {
			if (dmz_write_super(dev, 0, dev->bdev[i].block_offset) < 0)
				return -1;
		}
	}
