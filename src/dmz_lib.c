...
int dmz_reset_zone(struct dmz_dev *dev, struct blk_zone *zone) {
  ...
  if (ioctl(bdev->fd, BLKRESETZONE, &range) < 0) {
    fprintf(stderr, "%s: Reset zone %u failed %d (%s)\n", bdev->name,
            dmz_zone_id(dev, zone), errno, strerror(errno));
    return -1;
  }
...
int dmz_locate_metadata(struct dmz_dev *dev) {
  ...
  nr_bitmap_zones =
      (dev->nr_bitmap_blocks + dev->zone_nr_blocks - 1) / dev->zone_nr_blocks;
  ...
  nr_meta_zones =
      ((nr_meta_blocks + dev->zone_nr_blocks - 1) / dev->zone_nr_blocks);
   ...
   */
  dev->nr_chunks =
      dev->nr_usable_zones - (dev->total_nr_meta_zones + dev->nr_reserved_seq);
  ...
  dev->nr_meta_zones =
      ((dev->nr_meta_blocks + dev->zone_nr_blocks - 1) / dev->zone_nr_blocks);
