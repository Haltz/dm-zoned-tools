...
int dmz_reset_zone(struct dmz_dev *dev, struct blk_zone *zone) {
	...
	if (dmz_zone_unknown(zone) || dmz_zone_conv(zone) || dmz_zone_empty(zone))
		return 0;
	...
	if (ioctl(bdev->fd, BLKRESETZONE, &range) < 0) {
		fprintf(stderr, "%s: Reset zone %u failed %d (%s)\n", bdev->name, dmz_zone_id(dev, zone), errno, strerror(errno));
		return -1;
	}
...
int dmz_locate_metadata(struct dmz_dev *dev) {
	...
	for (i = 0; i < dev->nr_zones; i++) {
		...
		if (dmz_zone_is_cache(dev, zone)) {
			if (dev->sb_zone == NULL) {
				...
				fprintf(stdout, "SuperBlockZone is %d\n", i);
   ...
   */
	if (dev->nr_cache_zones < 3) {
		fprintf(stderr, "%s: Not enough random zones found\n", dev->label);
		return -1;
	}
   ...
   */
	...
	if (dev->nr_reserved_seq >= dev->nr_usable_zones) {
		fprintf(stderr, "%s: Not enough usable zones found\n", dev->label);
		return -1;
	}
   ...
   */
	nr_chunks = dev->nr_usable_zones - (nr_bitmap_zones + dev->nr_reserved_seq);
   ...
   */
	...
	nr_meta_zones = ((nr_meta_blocks + dev->zone_nr_blocks - 1) / dev->zone_nr_blocks);
	...
	if (dev->total_nr_meta_zones > dev->nr_cache_zones) {
		fprintf(stderr,
			...
			"(need %u, have %u)\n",
			dev->label, dev->total_nr_meta_zones, dev->nr_cache_zones);
		return -1;
	}
   ...
   */
	dev->nr_chunks = dev->nr_usable_zones - (dev->total_nr_meta_zones + dev->nr_reserved_seq);
	...
	dev->nr_meta_zones = ((dev->nr_meta_blocks + dev->zone_nr_blocks - 1) / dev->zone_nr_blocks);
