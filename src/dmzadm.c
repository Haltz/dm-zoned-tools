...
void print_dev_info(struct dmz_block_dev *bdev) {
	printf("%s: %llu 512-byte sectors (%llu GiB)\n", bdev->path, bdev->capacity, (bdev->capacity << 9) / (1024ULL * 1024ULL * 1024ULL));
...
int main(int argc, char **argv) {
	...
	for (i = optnum; i < argc; i++) {
		if (strcmp(argv[i], "--verbose") == 0) {
			...
			if (op != DMZ_OP_FORMAT) {
				fprintf(stderr, "--seq option is valid only with the "
						"format operation\n");
				return 1;
			}
			...
			if (dev->nr_reserved_seq <= 0) {
				fprintf(stderr, "Invalid number of sequential zones\n");
				return 1;
			}
			...
			if (op != DMZ_OP_FORMAT) {
				fprintf(stderr, "--label option is valid only with the "
						"format operation\n");
				return 1;
			}
			if (label[0] == '\'' || label[0] == '\"') {
			...
			if (label_size > 31) {
				fprintf(stderr, "Label too long (max 16 characters)\n");
				return 1;
			}
			...
			if (op != DMZ_OP_FORMAT) {
				fprintf(stderr, "--force option is valid only with the "
						"format operation\n");
				return 1;
			}
	...
	if (dev->nr_bdev > 1) {
		if (dmz_bdev_is_zoned(&dev->bdev[0])) {
			fprintf(stderr, "%s: Not a regular block device\n", dev->bdev[0].name);
			dmz_close_dev(&dev->bdev[0]);
			return 1;
		}
	} else {
		if (!dmz_bdev_is_zoned(&dev->bdev[0])) {
			fprintf(stderr, "%s: Not a zoned block device\n", dev->bdev[0].name);
			dmz_close_dev(&dev->bdev[0]);
			return 1;
		}
	...
	if (dev->nr_bdev > 1) {
		fprintf(stderr, "Haltz not support multiple devices");
		return 1;
	}
	...
	// 	if (dev->zone_nr_sectors && dev->zone_nr_sectors != dev->bdev[i].zone_nr_sectors) {
	// 		fprintf(stderr, "%s: zone_nr_sectors mismatch (%lu/%lu)\n", dev->bdev[i].name, dev->zone_nr_sectors, dev->bdev[i].zone_nr_sectors);
	// 		ret = 1;
	// 		goto out_close;
	// 	} else
	// 		dev->zone_nr_sectors = dev->bdev[i].zone_nr_sectors;
	// 	if (dev->zone_nr_blocks && dev->zone_nr_blocks != dev->bdev[i].zone_nr_blocks) {
	// 		fprintf(stderr, "%s: zone_nr_blocks mismatch (%lu/%lu)\n", dev->bdev[i].name, dev->zone_nr_blocks, dev->bdev[i].zone_nr_blocks);
	// 		ret = 1;
	// 		goto out_close;
	// 	} else
	...
	// 	dev->bdev[0].nr_zones = dev->bdev[0].capacity / dev->zone_nr_sectors;
	...
	// 		block_offset += dev->bdev[i].nr_zones * dev->zone_nr_blocks;
	// 	}
	// }
